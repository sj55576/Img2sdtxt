...
#### `sd_generate_multi_model(request_data) → dict`

`POST /api/sd/generate-multi-model` (JSON)

| 項目 | 内容 |
|------|------|
| **引数** | `models: list` — 生成に使用するモデル名のリスト（必須）<br>`positive/negative: str` — プロンプト<br>その他は `sd_generate` と同様 |
| **処理内容** | モデルリストを指定して、各モデルで順序に txt2img 生成を実行。各モデルごとに結果オブジェクト（成功時はファイル情報、失敗時はエラー）を生成し、全体結果リストを返す。 |
...
| 項目 | 内容 |
|------|------|
| **処理内容** | `feature` が `{generate, sd, img2img, inpaint, multi_model}` のいずれかであることを検証後、`_read_last_params()` から対応するパラメータを返す。 |
...
| 項目 | 内容 |
|------|------|
| **処理内容** | `feature` を検証後（有効値: `{generate, sd, img2img, inpaint, multi_model}`）、`_read_last_params()` で現在のデータを読み込み、`feature` キーに `request_data` をセットして `_write_last_params()` で保存する。 |
