...
def get_llm_cache_identities() -> list[tuple[str, str]]:
    """Return cache identities in fallback order.

    A fallback wrapper has no single provider identity before a request runs.
    Looking up each concrete provider key avoids treating a fallback response
    as if it had been generated by the primary provider.
    """
    client = llm_client
    if isinstance(client, FallbackChain):
        return [(provider.provider_name, provider.model) for provider in client.providers]

    provider_name = getattr(client, "provider_name", "")
    model = getattr(client, "model", "")
    return [
        (
            provider_name if isinstance(provider_name, str) else "",
