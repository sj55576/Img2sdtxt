...
| `WILDCARD_BATCH_MAX_COMBINATIONS` | `36` | Maximum images generated by one wildcard "all combinations" batch job |
...
Prompts (in Generate, SD Generate, Img2Img, Inpaint, and XY Plot) can include
`sd-dynamic-prompts`-style syntax, expanded by `dynamic_prompts.py`:

| Syntax | Meaning |
|--------|---------|
| `{a\|b\|c}` | Pick one option at random (nesting allowed, e.g. `{a\|{b\|c}}`) |
| `__filename__` | Pick one random line from `data/wildcards/filename.txt` |
| `\{` `\}` `\|` | Escaped literals, kept as-is |

Manage wildcard files from the **Wildcards** page (`GET/POST /api/wildcards/`,
`GET/PUT/DELETE /api/wildcards/{name}`), which also lets you preview expansions
and count the total number of combinations a template can produce
(`POST /api/wildcards/expand`).

There are three ways to turn a template into images:

- **Once per generated image** — enable "Expand dynamic prompt syntax once per
  generated image" (`expand_wildcards` on `SDGenerateRequest` /
  `SDMultiModelRequest`) on the SD Generate / Img2Img / Inpaint pages, or the
  🃏 **Expand** button next to the Positive Prompt field to expand it once,
  inline, before generating. Each image in the batch gets its own expansion,
  and the original template (not the expansion) is stored alongside the result
  so history keeps both.
- **All combinations, text only** — the Wildcards page's "Preview" and "Count
  combinations" buttons call `POST /api/wildcards/expand` for a quick look at
  what a template can produce, without generating any images.
- **All combinations, as images** — the Wildcards page's "🎲 Generate all
  combinations" button submits a `wildcard_batch` job (`POST /api/jobs/submit`)
  that generates one image per combination. The combination count is checked
  against `WILDCARD_BATCH_MAX_COMBINATIONS` before the job is queued, the same
  way an oversized XY Plot grid is rejected before it starts.
...
| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/api/jobs/submit` | Submit `txt2img`, `multi_model`, `xy_plot`, or `wildcard_batch` job |
